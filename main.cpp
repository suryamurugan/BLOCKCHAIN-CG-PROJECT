/*******************************************************************************************************
	PROJECT NAME : BLOCKCHAIN VISUALIZATION USING OPENGL
	AUTHORS : SURYA MURUGAN & NIDA WAJID
	PROJECT INIT : 04/04/2019
...
// initializing
int i;
char *str1 = "ATRIA INSTITUTE OF TECCHNOLOGY";
...
char *str8 = "SURYA M - (1AT16CS111)";
char *str9 = "NIDA WAZID - (1AT16CS067)";
char *str10 = "GUIDES:";
char *str11 = "PROF. VIJAY SWAROOP"
char *str12 = "ASST PROF. GAUTHAM";
char *str13 = "********************************************* MENU *********************************************";
...
void display1()
{
	...
	for (i = 0;i < strlen(str1);i++)
		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, str1[i]); //P.A. COLLEGE OF ENGINEERING, MANGALORE
	glRasterPos2f(-5, 6);
	...
	for (i = 0;i<strlen(str8);i++)
		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, str8[i]); //NIKHIL AMIN
	glRasterPos2f(-8.5, -7);
	for (i = 0;i<strlen(str9);i++)
		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, str9[i]); //(4PA15CS073)
	glRasterPos2f(5, -5);
	for (i = 0;i<strlen(str10);i++)
		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, str10[i]); //GUIDES:
	glRasterPos2f(5.5, -6);
	for (i = 0;i<strlen(str11);i++)
		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, str11[i]); //PROF. NAMRATHA RAI
	glRasterPos2f(5.5, -7);
	for (i = 0;i<strlen(str12);i++)
		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, str12[i]); //MS. NASEEBA
	glColor3f(1, 0, 0);
	glFlush();
	glRasterPos2f(-2.5, -9);
...
void display2()
{
	...
	for (i = 0;i<strlen(str17);i++)
		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, str17[i]); //Press (4) : Do you need a Blockchain?
	glRasterPos2f(-3, -4);
...
//3rd block
	...
	glFlush();for (int i = 0; i < 40000; i++) for (int j = 0; j < 30000; j++); //delay
...
//3rd block
	...
	glFlush();for (int i = 0; i < 20000; i++) for (int j = 0; j < 20000; j++); //delay
...
//block3 of computer2
	...
	glFlush();
	for (int i = 0; i < 25000; i++) for (int j = 0; j < 20000; j++); //delay
